...
- שמות בעברית מדוברת ישראלית בלבד — אותיות עבריות בלבד, אסור בשום פנים להשתמש בתווים מכל שפה אחרת.`,
    ...
    const nameMsg = await anthropic.messages.create({
      ...
      max_tokens: 40,
      temperature: 0.3,
      system: `אתה מומחה במטבח ישראלי. קבל רשימת מרכיבים והחזר שם מנה קצר (עד 5 מילים) בעברית מדוברת יומיומית.
כללים:
- עברית טבעית כפי שישראלי ממוצע אומר בפה: "פירה", "שניצל עוף", "סלט ירקות"
- מנות בינלאומיות מוכרות: השתמש בשמן העברי המקובל: "וולינגטון", "בורגיניון", "ריזוטו", "קיש", "פסטה קרבונרה"
- אל תמציא תעתיקים חדשים — אם לא בטוח, תאר בעברית: "פילה בקר בבצק עלים"
- פלט שם בלבד, ללא הסבר`,

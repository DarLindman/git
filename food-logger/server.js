...
app.post('/api/analyze', auth, analyzeLimiter, async (req, res) => {
  ...
  try {
    const message = await anthropic.messages.create({
      ...
      max_tokens: 700,
      temperature: 0.3,
      system: 'אתה מנתח תזונה. זהה כל מרכיב בתמונה בנפרד. לכל מרכיב קבע weight_g קודם ואז חשב ערכים עקביים. הנח מנת מסעדה: שמן, חמאה, רטבים — הכל כלול. אל תמעיט. שמות בעברית מדוברת ישראלית בלבד — אותיות עבריות בלבד, אסור בשום פנים להשתמש בתווים מכל שפה אחרת (אנגלית, סינית, ערבית, יפנית וכו\'). דוגמאות: "פירה" ולא "תפוח אדמה מעוך", "צ\'יפס" ולא "תפוחי אדמה מטוגנים", "חרדל" ולא 芥末.',
      messages: [{
        role: 'user',
        content: [
          {

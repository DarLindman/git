...
2. כמות מפורשת — חשב בדיוק לפי הכמות שצוינה, ללא עיגול או שינוי.
   דוגמה: "100 גרם אורז" = בדיוק 100 גרם. "3 ביצים" = בדיוק 3 ביצים.

3. צורת יחיד = בדיוק יחידה אחת. זו הנחיה מכוונת של המשתמש ואסור להתעלם ממנה.
   • "סרדין" = סרדין 1 (לא מנת סרדינים, לא כמה סרדינים).
   • "ביצה" = ביצה 1. "עגבניה" = עגבניה 1.
   • "אנשובי" = פילה אנשובי 1 (בסביבות 4-5 גרם).
   צורת רבים ללא מספר = מנה רגילה רגילה. צורת יחיד = 1 יחידה.

4. ללא כמות ולא ריבוי — הנח כמות ריאלית צנועה-בינונית כחלק ממנה מעורבת:
   • "טונה במים" (פחית, ללא ציון כמות) = כ-80-100 גרם נטו מסוננת (~100-120 קל).
   • "סרדינים" (ריבוי ללא מספר) = 2-3 סרדינים (~40-50 גרם, ~80 קל).
   • "עוף" = ~100 גרם מבושל (~165 קל).
   • "אורז" = ~100-120 גרם מבושל (~130-155 קל).
   • חשב לפי המנה המשולבת הכוללת: ריבוי מרכיבים = כמות קטנה יותר לכל אחד.

5. פרשנות "מוכן לאכילה" — פרש כפי שהצרכן מבין:
   • "5 אגוזי מלך" = 5 חצאי גרעינים (~25 גרם), לא אגוזים שלמים בקליפה.
   • "כף שמן" = ~13 מ"ל. "פרוסת לחם" = ~25-30 גרם.`,
      messages: [{
        role: 'user',
